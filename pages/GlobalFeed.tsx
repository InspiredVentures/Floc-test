...
import { MOCK_GLOBAL_POSTS } from '../data/mockPosts';
import { getCommunityFromPost } from '../lib/communityHelpers';
...
const GlobalFeed: React.FC = () => {
  ...
  const filteredPosts = useMemo(() => {
    ...
    return MOCK_GLOBAL_POSTS.filter(p => {
      const community = getCommunityFromPost(p);
      const communityTitle = community.title || '';

      return (
        p.content.toLowerCase().includes(q) ||
        p.author.toLowerCase().includes(q) ||
        communityTitle.toLowerCase().includes(q)
      );
    });
  }, [searchQuery]);
