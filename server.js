...
// WeTravel Proxy Endpoints

// Get Trip
app.get('/api/wetravel/trips/:id', async (req, res) => {
  if (!weTravelApiKey) {
      return res.status(503).json({ error: 'WeTravel API Key missing' });
  }
  try {
      const response = await fetch(`${WETRAVEL_API_BASE}/trips/${req.params.id}`, {
          headers: {
